...
void right_left_stop(int left,int right,GPIO_PinState pinstate)
{
		...
		if(pinstate==GPIO_PIN_SET)
		{
			...
			//a left white
			if(left<=0) GPIO_WritePin(GPIOA, GPIO_PIN_0,GPIO_PIN_SET);
			if(left<=1) GPIO_WritePin(GPIOA, GPIO_PIN_1,GPIO_PIN_SET);
			if(left<=2) GPIO_WritePin(GPIOA, GPIO_PIN_4,GPIO_PIN_SET);
		}
...
void upper_lower_stop(int up,int down,GPIO_PinState pinstate)
{
		...
		{
				//B upper white
			if(up<=0) GPIO_WritePin(GPIOB, GPIO_PIN_0,GPIO_PIN_SET);
			if(up<=1) GPIO_WritePin(GPIOB, GPIO_PIN_1,GPIO_PIN_SET);
			if(up<=2) GPIO_WritePin(GPIOB, GPIO_PIN_2,GPIO_PIN_SET);
